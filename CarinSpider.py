...
class createObjForStep:
	def __init__(step,zeroLevel,delayTime):
		self.step = step
		self.zeroLevel = zeroLevel
		self.delayTime = delayTime

class spiderMove:
  	def __init__(footPart1S,footPart2S,footPart3S,servoMaxInPWM,servoMinInPWM,high,highMax,highMin):
	...
	def attribution(footNum,objInfo,mirrorProperty):
	...
	def forward():
		...
		self.foot(1,objInfoForA,True)
		self.foot(3,objInfoForA,False)
		self.foot(5,objInfoForB,True)
		time.sleep(objInfo.delayTime*2)
		self.foot(2,objInfoForA,False)
		self.foot(4,objInfoForB,False)
		self.foot(6,objInfoForB,True)
	...
	def footStep(footNum,objInfo,mirrorProperty):
		while self.breakCall :
