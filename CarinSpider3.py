...
class spiderMove:
	...
	def attribution(self,footNum,objInfo,mirrorProperty):
        ...
        def xAxisStep(self,direction):
                zeroLevel = self.highControl(self.high)
                stepA  = self.xAxisStepControl(5).A
                delayTime = 1000
                stepB = self.xAxisStepControl(5).B
                ...
                if direction == "left" :
                    ...
                    self.footStep(1,objInfoForB,True)
                    self.footStep(3,objInfoForB,False)
                    self.footStep(5,objInfoForA,True)
                    time.sleep(objInfo.delayTime*2)
                    self.footStep(2,objInfoForB,False)
                    self.footStep(4,objInfoForA,False)
                    self.footStep(6,objInfoForA,True)
                
	def yAxisStep(self,direction):
		zeroLevel = self.highControl(self.high)
	...
	def footStep(self,footNum,objInfo,mirrorProperty):
		while self.breakCall :
			print(objInfo.zeroLevel.Alfa)
                        print(objInfo.zeroLevel.Betta)
                        print(objInfo.zeroLevel.Gamma)
...
spider = spiderMove(2.5,5.5,8,2.5,12.5,10,15,5)
cap = cv2.VideoCapture(0)
async def hello(uri):
    async with websockets.connect(uri) as websocket:
        while True: 
            shapeCaseWord = await websocket.recv()
            retval, image = cap.read()
            retval, buffer = cv2.imencode('.jpg', image)
            jpg_as_text = base64.b64encode(buffer)
            await websocket.send(jpg_as_text)
            if shapeCaseWord :
                if shapeCaseWord == "forward":
                    spider.yAxisStep('forward')
                elif shapeCaseWord == "backward":
                    spider.yAxisStep('backward')
                elif shapeCaseWord == "left":
                    spider.xAxisStep('left')
                elif shapeCaseWord == "right":
                    spider.xAxisStep('right')
                elif shapeCaseWord == "start":
                    spider.start()
        cap.release()   
